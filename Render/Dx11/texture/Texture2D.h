...
public:
    ...
    virtual ~Texture2DBase() {
        ...
        m_pTextureSRV.ReleaseAndGetAddressOf();
    }
...
public:
    Texture2DMS(ID3D11Device* device, uint32_t width, uint32_t height, DXGI_FORMAT format,
                ...
                uint32_t bindFlags = D3D11_BIND_RENDER_TARGET | D3D11_BIND_SHADER_RESOURCE);
    ~Texture2DMS() {
        m_pTextureRTV.ReleaseAndGetAddressOf();
    }
...
public:
    ...
    ID3D11RenderTargetView* GetRenderTarget(size_t arrayIdx) {
        return m_pRenderTargetElements[arrayIdx].Get();
    }

    // RWTexture2D
    ID3D11UnorderedAccessView* GetUnorderedAccess(size_t arrayIdx) {
        return m_pUnorderedAccessElements[arrayIdx].Get();
    }

    // TextureCube
    using Texture2DBase::GetShaderResource;
    // Texture2D
    ID3D11ShaderResourceView* GetShaderResource(size_t arrayIdx) {
        return m_pShaderResourceElements[arrayIdx].Get();
    }
...
private:
    uint32_t m_MipLevels = 1;
    Microsoft::WRL::ComPtr<ID3D11RenderTargetView> m_pTextureArrayRTV;  // RTV指向纹理数组
    std::vector<Microsoft::WRL::ComPtr<ID3D11RenderTargetView>> m_pRenderTargetElements;
    std::vector<Microsoft::WRL::ComPtr<ID3D11UnorderedAccessView>> m_pUnorderedAccessElements;
    std::vector<Microsoft::WRL::ComPtr<ID3D11ShaderResourceView>> m_pShaderResourceElements;
};
...
private:
    Microsoft::WRL::ComPtr<ID3D11DepthStencilView> m_pTextureDSV;
};

class Depth2DMS : public Texture2DBase {
public:
    Depth2DMS(ID3D11Device* device, uint32_t width, uint32_t height, const DXGI_SAMPLE_DESC& sampleDesc,
              DepthStencilBitsFlag depthStencilBitsFlag = DepthStencilBitsFlag::Depth_24Bits_Stencil_8Bits,
              uint32_t bindFlags = D3D11_BIND_DEPTH_STENCIL | D3D11_BIND_SHADER_RESOURCE);
    ~Depth2DMS() {
        m_pTextureDSV.ReleaseAndGetAddressOf();
    }
...
public:
    ...
    ID3D11DepthStencilView* GetDepthStencil(size_t arrayIdx) {
        return m_pDepthStencilElements[arrayIdx].Get();
    }
...
private:
    uint32_t m_ArraySize = 1;
    Microsoft::WRL::ComPtr<ID3D11DepthStencilView> m_pDepthArrayDSV;
    std::vector<Microsoft::WRL::ComPtr<ID3D11DepthStencilView>> m_pDepthStencilElements;
    std::vector<Microsoft::WRL::ComPtr<ID3D11ShaderResourceView>> m_pShaderResourceElements;
};
...
public:
    Depth2DMSArray(
        ID3D11Device* device, uint32_t width, uint32_t height, uint32_t arraySize,
        const DXGI_SAMPLE_DESC& sampleDesc,
        DepthStencilBitsFlag depthStencilBitsFlag = DepthStencilBitsFlag::Depth_24Bits_Stencil_8Bits,
        uint32_t bindFlags = D3D11_BIND_DEPTH_STENCIL | D3D11_BIND_SHADER_RESOURCE);

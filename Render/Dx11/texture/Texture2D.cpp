...
std::string intToHexSprintf(HRESULT value) {
    char buffer[32] = {0};
...
Texture2DBase::Texture2DBase(ID3D11Device* device, const CD3D11_TEXTURE2D_DESC& texDesc,
    ...
    auto hr = device->CreateTexture2D(&texDesc, nullptr, m_pTexture.GetAddressOf());
    if (!m_pTexture) {
        LOGE << "create text error!!" << hr << "width = " << texDesc.Width << "height = " << texDesc.Height;
        m_pTexture.Reset();
        throw std::logic_error("create text error!!" + intToHexSprintf(hr));
    }
    // 用实际产生的mip等级等数据更新
    ...
    if ((desc.BindFlags & D3D11_BIND_SHADER_RESOURCE)) {
        hr = device->CreateShaderResourceView(m_pTexture.Get(), &srvDesc, m_pTextureSRV.GetAddressOf());
    }
    if (!m_pTextureSRV) {
        m_pTextureSRV.Reset();
        LOGE << "create text m_pTextureSRV error!!" << hr;
        throw std::logic_error("create text m_pTextureSRV error!!" + hr);
    }
}
...
Texture2D::Texture2D(ID3D11Device* device, uint32_t width, uint32_t height, DXGI_FORMAT format,
                     uint32_t mipLevels, uint32_t bindFlags)
...
Texture2DMS::Texture2DMS(ID3D11Device* device, uint32_t width, uint32_t height, DXGI_FORMAT format,
                         const DXGI_SAMPLE_DESC& sampleDesc, uint32_t bindFlags)
    : Texture2DBase(device,
                    CD3D11_TEXTURE2D_DESC(format, width, height, 1, 1, bindFlags, D3D11_USAGE_DEFAULT, 0,
...
TextureCube::TextureCube(ID3D11Device* device, uint32_t width, uint32_t height, DXGI_FORMAT format,
    ...
    if (bindFlags & D3D11_BIND_RENDER_TARGET) {
        ...
        for (uint32_t i = 0; i < 6; ++i) {
            CD3D11_RENDER_TARGET_VIEW_DESC rtvDesc(D3D11_RTV_DIMENSION_TEXTURE2DARRAY, format,
                                                   0,    // Mips
                                                   i, 1  // Array
            );
    ...
    if (bindFlags & D3D11_BIND_UNORDERED_ACCESS) {
        // 单个子资源
        for (uint32_t i = 0; i < 6; ++i) {
            CD3D11_UNORDERED_ACCESS_VIEW_DESC uavDesc(D3D11_UAV_DIMENSION_TEXTURE2DARRAY, format,
                                                      0,    // Mips
                                                      i, 1  // Array
            );
    ...
    if (bindFlags & D3D11_BIND_SHADER_RESOURCE) {
        ...
        for (uint32_t i = 0; i < 6; ++i) {
            CD3D11_SHADER_RESOURCE_VIEW_DESC srvElementDesc(
                D3D11_SRV_DIMENSION_TEXTURE2DARRAY, format, 0, (uint32_t)-1, i, 1  // Array
            );
...
void TextureCube::SetDebugObjectName(std::string_view name) {
    ...
    for (size_t i = 0; i < m_pRenderTargetElements.size(); ++i)
        ::SetDebugObjectName(m_pRenderTargetElements[i].Get(),
                             std::string(name) + ".RTV[" + std::to_string(i) + "]");
    for (size_t i = 0; i < m_pShaderResourceElements.size(); ++i)
        ::SetDebugObjectName(m_pShaderResourceElements[i].Get(),
                             std::string(name) + ".SRV[" + std::to_string(i) + "]");
    for (size_t i = 0; i < m_pUnorderedAccessElements.size(); ++i)
        ::SetDebugObjectName(m_pUnorderedAccessElements[i].Get(),
                             std::string(name) + ".UAV[" + std::to_string(i) + "]");
#else
    UNREFERENCED_PARAMETER(name);
#endif
}

Texture2DArray::Texture2DArray(ID3D11Device* device, uint32_t width, uint32_t height, DXGI_FORMAT format,
                               uint32_t arraySize, uint32_t mipLevels, uint32_t bindFlags)
    : Texture2DBase(device, CD3D11_TEXTURE2D_DESC(format, width, height, arraySize, mipLevels, bindFlags),
...
Texture2DMSArray::Texture2DMSArray(ID3D11Device* device, uint32_t width, uint32_t height, DXGI_FORMAT format,
                                   uint32_t arraySize, const DXGI_SAMPLE_DESC& sampleDesc, uint32_t bindFlags)
    : Texture2DBase(device,
                    CD3D11_TEXTURE2D_DESC(format, width, height, arraySize, 1, bindFlags, D3D11_USAGE_DEFAULT,
                                          0, sampleDesc.Count, sampleDesc.Quality),
    ...
    if (bindFlags & D3D11_BIND_RENDER_TARGET) {
        // 单个子资源
        for (uint32_t i = 0; i < arraySize; ++i) {
            CD3D11_RENDER_TARGET_VIEW_DESC rtvDesc(D3D11_RTV_DIMENSION_TEXTURE2DMSARRAY, format,
                                                   0,    // Mips
                                                   i, 1  // Array
            );
    ...
    if (bindFlags & D3D11_BIND_SHADER_RESOURCE) {
        ...
        for (uint32_t i = 0; i < arraySize; ++i) {
            CD3D11_SHADER_RESOURCE_VIEW_DESC srvElementDesc(
                D3D11_SRV_DIMENSION_TEXTURE2DMSARRAY, format, 0, (uint32_t)-1, i, 1  // Array
            );
...
Depth2DMS::Depth2DMS(ID3D11Device* device, uint32_t width, uint32_t height,
                     const DXGI_SAMPLE_DESC& sampleDesc, DepthStencilBitsFlag depthStencilBitsFlag,
                     uint32_t bindFlags)
    : Texture2DBase(
          device,
          CD3D11_TEXTURE2D_DESC(GetDepthTextureFormat(depthStencilBitsFlag), width, height, 1, 1, bindFlags,
                                D3D11_USAGE_DEFAULT, 0, sampleDesc.Count, sampleDesc.Quality),
          CD3D11_SHADER_RESOURCE_VIEW_DESC(D3D11_SRV_DIMENSION_TEXTURE2DMS,
                                           GetDepthSRVFormat(depthStencilBitsFlag))),
      m_MsaaSamples(sampleDesc.Count) {
    if (bindFlags & D3D11_BIND_DEPTH_STENCIL) {
...
Depth2DArray::Depth2DArray(ID3D11Device* device, uint32_t width, uint32_t height, uint32_t arraySize,
                           DepthStencilBitsFlag depthStencilBitsFlag, uint32_t bindFlags)
    : Texture2DBase(device,
                    CD3D11_TEXTURE2D_DESC(GetDepthTextureFormat(depthStencilBitsFlag), width, height,
                                          arraySize, 1, bindFlags),
                    CD3D11_SHADER_RESOURCE_VIEW_DESC(D3D11_SRV_DIMENSION_TEXTURE2DARRAY,
                                                     GetDepthSRVFormat(depthStencilBitsFlag))),
      m_ArraySize(arraySize) {
    if (bindFlags & D3D11_BIND_DEPTH_STENCIL) {
        // 单个子资源
        for (uint32_t i = 0; i < arraySize; ++i) {
            CD3D11_DEPTH_STENCIL_VIEW_DESC dsvElementDesc(D3D11_DSV_DIMENSION_TEXTURE2DARRAY,
                                                          GetDepthDSVFormat(depthStencilBitsFlag), 0, i,
                                                          1);  // Array
        ...
        // 完整子资源
        CD3D11_DEPTH_STENCIL_VIEW_DESC dsvDesc(D3D11_DSV_DIMENSION_TEXTURE2DARRAY,
    ...
    if (bindFlags & D3D11_BIND_SHADER_RESOURCE) {
        // 单个子资源
        for (uint32_t i = 0; i < arraySize; ++i) {
            CD3D11_SHADER_RESOURCE_VIEW_DESC srvElementDesc(D3D11_SRV_DIMENSION_TEXTURE2DARRAY,
                                                            GetDepthSRVFormat(depthStencilBitsFlag), 0, 1, i,
                                                            1);  // Array
...
void Depth2DArray::SetDebugObjectName(std::string_view name) {
    ...
    for (size_t i = 0; i < m_pDepthStencilElements.size(); ++i)
        ::SetDebugObjectName(m_pDepthStencilElements[i].Get(),
                             std::string(name) + ".DSV[" + std::to_string(i) + "]");
    for (size_t i = 0; i < m_pShaderResourceElements.size(); ++i)
        ::SetDebugObjectName(m_pShaderResourceElements[i].Get(),
                             std::string(name) + ".SRV[" + std::to_string(i) + "]");
#else
...
Depth2DMSArray::Depth2DMSArray(ID3D11Device* device, uint32_t width, uint32_t height, uint32_t arraySize,
    ...
    : Texture2DBase(
          device,
          CD3D11_TEXTURE2D_DESC(GetDepthTextureFormat(depthStencilBitsFlag), width, height, arraySize, 1,
                                bindFlags, D3D11_USAGE_DEFAULT, 0, sampleDesc.Count, sampleDesc.Quality),
          CD3D11_SHADER_RESOURCE_VIEW_DESC(D3D11_SRV_DIMENSION_TEXTURE2DMSARRAY,
                                           GetDepthSRVFormat(depthStencilBitsFlag))),
      m_ArraySize(arraySize),
      m_MsaaSamples(sampleDesc.Count) {
    if (bindFlags & D3D11_BIND_DEPTH_STENCIL) {
        // 单个子资源
        for (uint32_t i = 0; i < arraySize; ++i) {
            CD3D11_DEPTH_STENCIL_VIEW_DESC dsvElementDesc(D3D11_DSV_DIMENSION_TEXTURE2DMSARRAY,
                                                          GetDepthDSVFormat(depthStencilBitsFlag), 0, i,
                                                          1);  // Array
        ...
        // 完整子资源
        CD3D11_DEPTH_STENCIL_VIEW_DESC dsvDesc(D3D11_DSV_DIMENSION_TEXTURE2DMSARRAY,
                                               GetDepthDSVFormat(depthStencilBitsFlag), 0);
        device->CreateDepthStencilView(m_pTexture.Get(), &dsvDesc, m_pDepthArrayDSV.GetAddressOf());
    }
    ...
    if (bindFlags & D3D11_BIND_SHADER_RESOURCE) {
        // 单个子资源
        for (uint32_t i = 0; i < arraySize; ++i) {
            CD3D11_SHADER_RESOURCE_VIEW_DESC srvElementDesc(D3D11_SRV_DIMENSION_TEXTURE2DMSARRAY,
                                                            GetDepthSRVFormat(depthStencilBitsFlag), 0, 1, i,
                                                            1);  // Array
